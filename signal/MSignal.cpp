...
int MSignal::init_signal()
{
    ...
    if (sigprocmask(SIG_BLOCK, &set, nullptr) == -1) {
        logger->logSystem(LogLevel::ERROR, "sigprocmask()失败!");
        return -1;
    }
    ...
    for (auto& sig : signals_) {
        ...
        if (sigaction(sig.signo, &sa, nullptr) == -1) {
            ...
            logger->logSystem(LogLevel::INFO, "Signal handler for %s registered successfully", sig.signame);
...
void MSignal::unmask_and_set_handler(int signo, SignalHandlerFunction handler)
{
    ...
    if (sigprocmask(SIG_UNBLOCK, &current_set, nullptr) == -1) {
        logger->logSystem(LogLevel::ERROR, "sigprocmask()解除信号失败!");
    }
    ...
    if (sigaction(signo, &sa, nullptr) == -1) {
        logger->logSystem(LogLevel::ERROR, "sigaction()设置信号处理失败: %d", signo);
    }
}
