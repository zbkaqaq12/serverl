...
#include <arpa/inet.h>
#include <netinet/tcp.h>    // 为了使用 TCP_KEEPIDLE 等宏
	...
	do
	{
		...
		if (s == -1)
		{
			...
			if (err == ECONNABORTED)  //ECONNRESET错误则发生在对方意外关闭套接字后，这里收到该消息，而我们继续处理--这里可能是对方断开了
			{
				...
				level = LogLevel::ERROR;
				logger->logSystem(level, "Connection aborted by client");
			}
			...
			{
				level = LogLevel::ERROR;
				logger->logSystem(level, "File descriptor limit reached");
			}
			logger->logSystem(level, "Accept failed: %s", strerror(err));
		...
		if (newc == nullptr)
		{
			...
			if (close(s) == -1)
			{
				logger->logSystem(LogLevel::ERROR, "CSocekt::ngx_event_accept()中close(%d)失败!", s);
			}
			return;
		}
		...
		// 初始化 HTTP 上下文
		newc->httpCtx = std::make_unique<connection_s::HttpContext>();
		newc->httpCtx->lastActivityTime = time(NULL);
		newc->httpCtx->state = connection_s::HttpContext::ConnectionState::WAITING;

		// 设置 TCP keepalive 参数
		int keepAlive = 1;           // 启用 keepalive 机制
		int keepIdle = 60;           // 空闲 60 秒后开始发送探测包
		int keepInterval = 5;        // 每 5 秒发送一次探测包
		int keepCount = 3;           // 连续 3 次探测失败后断开连接

		// 应用这些设置
		if (setsockopt(s, SOL_SOCKET, SO_KEEPALIVE, &keepAlive, sizeof(keepAlive)) < 0) {
			logger->logSystem(LogLevel::WARN, "Failed to set SO_KEEPALIVE");
		}
		setsockopt(s, IPPROTO_TCP, TCP_KEEPIDLE, &keepIdle, sizeof(keepIdle));      // 设置空闲时间
		setsockopt(s, IPPROTO_TCP, TCP_KEEPINTVL, &keepInterval, sizeof(keepInterval)); // 设置探测间隔
		setsockopt(s, IPPROTO_TCP, TCP_KEEPCNT, &keepCount, sizeof(keepCount));     // 设置探测次数

		// 修改 epoll 事件标志，使用边缘触发模式
		...
		if (epoll_oper_event(
			...
			events, //标志，这里代表要增加的标志,EPOLLIN：可读，EPOLLRDHUP：TCP连接的远端关闭或者半关闭 ，如果边缘触发模式可以增加 EPOLLET
