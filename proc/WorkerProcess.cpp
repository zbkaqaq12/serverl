...
void WorkerProcess::init_process()
{
    ...
    if (signalHandler_->init_signal() == -1) {
        Logger::GetInstance()->logSystem(LogLevel::ERROR, "信号初始化失败!");
        return;
    }
    ...
    // 初始化线程池，处理接收到的消息
    int tmpthreadnums = Config::GetInstance()->GetIntDefault("ProcMsgRecvWorkThreadCount", 5);
...
{
    Logger::GetInstance()->logSystem(LogLevel::INFO, "Worker process received SIGTERM, performing cleanup...");
    // 执行清理工作并退出
    exit(0);
}
...
{
    Logger::GetInstance()->logSystem(LogLevel::INFO, "Worker process received SIGINT, performing cleanup...");
    // 执行清理工作并退出
    exit(0);
}
...
{
    Logger::GetInstance()->logSystem(LogLevel::INFO, "Worker process received SIGHUP, reloading configuration...");
    // 重新加载配置等操作
}
...
{
    Logger::GetInstance()->logSystem(LogLevel::INFO, "Worker process received SIGUSR1, performing specific task...");
    // 执行用户自定义操作
}
...
{
    Logger::GetInstance()->logSystem(LogLevel::INFO, "Worker process received SIGUSR2, performing another task...");
    // 执行另个用户自定义操作
}
